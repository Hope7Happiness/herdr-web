...
function handle(req, res) {
  ...
  const up = http.request({ host: '127.0.0.1', port: t.port, method: req.method, path: t.path, headers }, (r) => {
    ...
    r.on('end', () => {
      ...
      const inject = `<base href="/p/${t.port}/">${shim(t.port)}`;
...
const NOISE = /^(adb|sshd?|cupsd|dnsmasq|systemd-resolve|tailscaled?|containerd|dockerd|postgres|mysqld|redis-server|mongod)$/i;
...
function listPorts(selfPort) {
  return new Promise((resolve) => {
    execFile('ss', ['-ltnpH'], { timeout: 4000 }, (err, stdout) => {
      if (err) return resolve([]);
      const seen = new Map();
      for (const line of stdout.split('\n')) {
        const addr = /\s(\S+):(\d+)\s+\S+\s/.exec(line);
        if (!addr) continue;
        const host = addr[1];
...
module.exports = { handle, handleUpgrade, listPorts, allow, isAllowed };
